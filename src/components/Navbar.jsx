  ...
  return (
    <nav className="sticky top-0 z-40 bg-cream/90 backdrop-blur-md">
      <div className="mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        {/* ── Logo ── */}
        <Link to="/" className="text-xl tracking-wider text-dark font-semibold">
          Kallee Nepal
        </Link>
          ...
          <div className="hidden md:flex items-center gap-8">
            {links.map((l) => (
              ...
              >
                {l.label}
                {l.badge > 0 && (
                  <span className="absolute -top-2 -right-4 bg-accent text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center">
                    {l.badge}
                  </span>
                )}
              </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          {user ? (
            <div className="relative">
              <button
                className="w-8 h-8 rounded-full bg-accent text-cream text-sm font-medium flex items-center justify-center"
              ...
              {dropdown && (
                ...
                >
                  <p className="px-4 py-1 text-sm text-muted truncate">
                    {user.first_name ||
                      user.firstName ||
                      user.name ||
                      user.email}
                  ...
                  <hr className="my-1" />
                  <button
                    className="w-full text-left px-4 py-2 text-sm text-dark hover:bg-cream transition"
                    onClick={() => { navigate("/profile/edit"); setDropdown(false); }}
                  >
                    Update user
                  </button>
                  <button
                    className="w-full text-left px-4 py-2 text-sm text-dark hover:bg-cream transition"
                    onClick={() => { navigate("/change-password"); setDropdown(false); }}
                  >
                    Update password
                  </button>
                  <hr className="my-1" />
