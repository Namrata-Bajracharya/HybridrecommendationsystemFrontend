...
const Checkout = () => {
  ...
  const handleCheckoutSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const userResponse = await customFetch.get(routesName.AuthRoute().me);
    const user = userResponse.data;

    const response = await customFetch.post(routesName.OrderRoute().create, {
      shipping_address_id: Number((checkoutData.data as Record<string, string>).address || 1),
      billing_address_id: Number((checkoutData.data as Record<string, string>).address || 1),
      checkoutData,
      user: {
        email: user.email,
        id: user.id,
