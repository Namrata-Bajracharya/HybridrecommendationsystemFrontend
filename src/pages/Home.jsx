...
export default function HomePage() {
  const { forYou, trending, recentlyViewed, occasionEdits } = useRecommendations()
      ...
      <section className="bg-cream-alt text-center">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
          <h1 className="text-4xl sm:text-5xl font-semibold text-dark mb-4 leading-tight">
            Timeless.<br />Handcrafted.<br />Yours.
          </h1>
          <p className="text-muted max-w-md mx-auto mb-6 text-sm">
            Discover ethnic wear that tells a story — from handloom sarees to hand-embroidered kurthas.
          </p>
          <Link to="/products"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {forYou.length > 0 && (
          <section className="mb-12">
            <h2 className="text-xl font-semibold text-dark mb-6">Recommended for You</h2>
        ...
        {occasionEdits.filter(o => o.items.length > 0).slice(0, 3).map(({ occasion, items }) => (
        ...
        {recentlyViewed.length > 0 && (
          <section className="mb-12">
            <h2 className="text-xl font-semibold text-dark mb-6">Recently Viewed</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <section className="bg-cream-alt p-8 sm:p-12 grid grid-cols-1 sm:grid-cols-3 gap-8 text-center">
          {[
            { emoji: '🧵', title: 'Handloom & Handcrafted', desc: 'Every piece supports traditional artisans across India.' },
            { emoji: '🌿', title: 'Mindfully Sourced', desc: 'Natural fabrics, ethical production, minimal waste.' },
            { emoji: '✨', title: 'Curated for You', desc: 'Smart recommendations based on your unique taste.' },
          ].map(item => (
            <div key={item.title}>
