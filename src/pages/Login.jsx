    ...
    <div className="max-w-md mx-auto py-20">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="relative">
          <input type={show ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-2 rounded pr-10" />
          <button type="button" onClick={() => setShow((p) => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted hover:text-dark" tabIndex={-1}>
            <EyeIcon open={show} />
          </button>
        </div>
        ...
        <p className="text-xs text-center">
          <Link to="/forgot-password" className="text-muted hover:text-dark">Forgot password?</Link>
        </p>
